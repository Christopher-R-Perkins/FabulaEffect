...
const copyFiles = async () => {
  const recursiveFileSearch = (
    dir: string,
    callback: (err: NodeJS.ErrnoException | null, res: Array<string>) => void
  ) => {
...
const buildWatch = () => {
  ...
  gulp.watch(
    ["Source/fonts", "Source/lang", "Source/templates", "Source/*.json"],
    { ignoreInitial: false },
    copyFiles
  );
};
...
const linkUserData = async () => {
  ...
  try {
    ...
    // I can't link the directory due to my Foundry being on a networked drive
    if (argv.clean || argv.c) {
      ...
      fs.cpSync(path.resolve("./dist"), linkDir, { recursive: true });
    } else {
      fs.rmSync(linkDir, { recursive: true, force: true });
      fs.cpSync(path.resolve("./dist"), linkDir, { recursive: true });
    }
...
const gitTaskBuild = (cb: gulp.TaskFunctionCallback) => {
  ...
  return gulp
    .src(`dist/${manifest.file.name}-v${manifest.file.version}.zip`)
